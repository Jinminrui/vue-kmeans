<template>
  <div class="components-container">
    <code><strong>上传文件并执行层次聚类算法</strong>
    </code>
    <el-upload
      ref="upload"
...
export default {
  ...
  methods: {
    ...
    drawChart() {
      const step = ecStat.clustering.hierarchicalKMeans(this.data, this.clusterNumber, true)
      var result
      const option = {
        timeline: {
          top: 'center',
      ...
      for (let i = 0; !(result = step.next()).isEnd; i++) {
        option.options.push(this.getOption(result, this.clusterNumber))
        option.timeline.data.push(i + '')
      }
      console.log(result)
      var myChart = echarts.init(document.getElementById('hierarchicalKMeans'))
